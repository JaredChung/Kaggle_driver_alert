...
def create_model_v1(img_rows, img_cols, color_type=1):
    ...
    model.add(Convolution2D(96, 3, 3, border_mode='same', init='he_normal'))
    model.add(Activation('relu'))
    model.add(Convolution2D(96, 3, 3, border_mode='valid', init='he_normal'))
    model.add(Activation('relu'))
    model.add(ZeroPadding2D(padding=(1, 1)))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Dropout(0.25))
...
def create_model_orignal(img_rows, img_cols, color_type=1):
    model = Sequential()
...
def run_cross_validation(nfolds=10):
    ...
    color_type_global = 3
    batch_size = 16
    nb_epoch = 50 # flat 10 is the best
    random_state = 51
